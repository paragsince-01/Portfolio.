...
export default function Home() {
  ...
  const targetTexts = ["Parag Aggarwal", "Web Developer"];
  const [currentIndex, setCurrentIndex] = useState(0);

  useEffect(() => {
    const targetText = targetTexts[currentIndex];
    let charIndex = 0;
    ...
    const intervalId = setInterval(() => {
      ...
      if (charIndex === targetText.length) {
        ...
        setTimeout(() => {
          setCurrentIndex((currentIndex + 1) % targetTexts.length);
        }, 1000); 
      }
    }, 150); 
      ...
      <div className='justify-start items-center pt-40 pb-12 md:py-40 px-10 md:px-20 w-full h-full flex flex-row bg-gray-200 rounded-xl relative top-10 mb-10'>
        <div className="flex flex-row relative bottom-5 md:bottom-0">
          ...
          <div className="flex flex-col homeAnimation">
            <h1 className='p-0 text-3xl md:text-7xl text-gray-800 font-bold tracking-wide'>I'm {text} <Cursor /></h1>
